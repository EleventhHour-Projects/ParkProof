...
export default function SelectedParkingLotPage() {
    ...
    try {
      ...
      if (res.ok) {
        ...
        const mapped: Vehicle[] = data.map((v: any) => ({
          ...
          type: v.type || '4w',
          icon: (v.type === '2w' || v.type === 'bike') ? '/bike.png' : '/car.png'
        }))
        setVehicles(mapped)

        if (mapped.length > 0 && !selectedVehicle) {
          // Try to find previously selected or default to first
          const storedId = localStorage.getItem('lastSelectedVehicle')
          const found = mapped.find(v => v._id === storedId)
          setSelectedVehicle(found || mapped[0])
        } else if (mapped.length > 0 && selectedVehicle) {
          // Ensure current selection is valid
          const stillExists = mapped.find(v => v._id === selectedVehicle._id)
          if (!stillExists) setSelectedVehicle(mapped[0])
          else setSelectedVehicle(stillExists)
        }
      }
    } catch (e) {
  ...
  const handleAddVehicle = async (e: React.FormEvent) => {
    e.preventDefault()
    if (!newVehicleNumber.trim() || !newVehicleName.trim()) {
  ...
  useEffect(() => {
    const handler = (e: MouseEvent) => {
      if (dropdownRef.current && !dropdownRef.current.contains(e.target as Node)) {
        setOpen(false)
        // setIsAddingVehicle(false) // Optionally keep open
      }
    }
  ...
  useEffect(() => {
    if (selectedVehicle) {
      ...
      // Use the actual type from DB if available, else fallback to model detection
      let type = '4w';
